...
import { auth, loginComGoogle, logout } from './firebase'
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4">
      <div className="w-full max-w-sm fade-up">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1.5">Faça login para acessar o painel</p>
        </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700/40 rounded-2xl p-6">
          <button
            onClick={entrar}
            disabled={carregando}
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 text-gray-700 font-medium text-sm px-4 py-3 rounded-xl transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
          >
            {carregando ? (
          ...
          {erro && (
            <p className="text-red-400 text-xs text-center mt-3">{erro}</p>
          )}
        </div>

        <p className="text-center text-slate-600 text-xs mt-5">
          Acesso restrito · {formatarData()}
        </p>
      </div>
    </div>
  )
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      {pagina === 'dashboard' ? <Dashboard onVerLista={() => setPagina('lista')} /> : <Lista />}
    </div>
  )
}
